...
namespace Advertise.Service.Validators.Accounts
{
    public class ForgotPasswordValidator : BaseValidator<ForgotPasswordViewModel>
    {
        public ForgotPasswordValidator(IUserService userService)
        {
            ...
            RuleFor(model => model.Email).MustAsync(userService.IsExistByEmailCancellationTokenAsync)
                .WithMessage("این پست الکترونیک ثبت نشده است");
            RuleFor(model => model.Email).MustAsync((username, token) => userService.IsEmailConfirmedByEmailAsync(username,http, token))
                .WithMessage("این پست الکترونیک تایید نشده است");
        }
    }
}
