...
namespace Advertise.Web.Controllers
{
    ...
    public partial class CompanyController : BaseController
    {
        ...
        [ValidateInput(false)]
        public virtual async Task<ActionResult> EditApprove(CompanyEditViewModel viewModel)
        {
        ...
        [ValidateAntiForgeryToken]
        [ValidateInput(false)]
        public virtual async Task<ActionResult> EditReject(CompanyEditViewModel viewModel)
        {
            if (viewModel == null)
                return View(MVC.Error.Views.BadRequest);
        ...
        [MvcSiteMapNode(ParentKey = "Panel_Home_Index", Title = "فروشگاه", Key = "Panel_Company_List")]
        public virtual async Task<ActionResult> List(CompanySearchRequest request)
        {
            var viewModel = await _companyFactory.PrepareListViewModelAsync(request);
            return View(MVC.Company.Views.List, viewModel);
        }
